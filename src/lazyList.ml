...
let rec take n xs =
  lazy
    (if n <= 0 then Nil
      ...
      match next xs with Nil -> Nil | Cons (y, ys) -> Cons (y, take (n - 1) ys))

let drop n xs =
  let rec loop n xs =
    if n <= 0 then next xs
    else match next xs with Nil -> Nil | Cons (_, ys) -> loop (n - 1) ys
  in
  lazy (loop n xs)
...
let rec zip_with f xs ys =
  lazy
    ...
    | Nil, _ -> Nil
    | _, Nil -> Nil
    | Cons (x, xss), Cons (y, yss) -> Cons (f x y, zip_with f xss yss))
...
let rec combine_with f xs ys =
  lazy
    ...
    | Cons (x, xss), Cons (y, yss) -> Cons (f x y, combine_with f xss yss))

let zip xs ys = zip_with (fun x y -> (x, y)) xs ys

let fold_right f b xs =
  let rec loop xs =
    match next xs with Nil -> b | Cons (x, xs) -> f x (lazy (loop xs))
  in
  loop xs
...
let fold_right1 f xs =
  let rec loop xs =
    ...
    | Cons (x, xs) -> (
        match next xs with Nil -> x | _ -> f x (lazy (loop xs)))
  in
  loop xs
...
let scan1 f xs =
  lazy (match next xs with Nil -> Nil | Cons (x, xs) -> next (scan f x xs))
...
let map_accum_l2 f acc xs =
  let g a (_, x) =
    ...
    (next c, (c, y))
  in
  let mapAcc = map_accum_l g acc (map (fun x -> (lazy acc, x)) xs) in
  (lazy (next (last (lazy acc ^:^ map fst mapAcc))), map snd mapAcc)

let flatten xss = List.fold_left ( ^@^ ) nil xss

let concat xss = lazy (fold_right (fun xs ys -> next (xs ^@^ ys)) Nil xss)
...
let inits xs =
  let rec loop xs ys =
    ...
    | Nil -> Nil
    | Cons (x, xs') ->
        let zs = ys ^@^ singleton x in
        Cons (zs, lazy (loop xs' zs))
  in
  nil ^:^ lazy (loop xs nil)

let rec tails xs =
  lazy (match next xs with Nil -> Nil | Cons (_, ys) -> Cons (xs, tails ys))
...
let difference p xs ys = filter (fun x -> not (any (p x) ys)) xs

let union ?(cmp = ( = )) xs ys = xs ^@^ difference cmp ys xs

let intersect ?(cmp = ( = )) xs ys = filter (fun x -> any (cmp x) ys) xs

let intersect_class ?(cmp = ( = )) xs ys =
  concat_map
    (fun x ->
      let ys' = filter (cmp x) ys in
      if is_empty ys' then nil else x ^:^ ys')
    xs
...
let subset ?(cmp = ( = )) xs ys = all (fun x -> any (cmp x) ys) xs
...
let partition p xs =
  let rec loop xs =
    match next xs with
    | Nil -> Nil
    | Cons (x', xs') when p x' -> Cons ((singleton x', nil), lazy (loop xs'))
    | Cons (x', xs') -> Cons ((nil, singleton x'), lazy (loop xs'))
  in
  (concat_map fst (lazy (loop xs)), concat_map snd (lazy (loop xs)))

let rec range m n = lazy (if m > n then Nil else Cons (m, range (m + 1) n))

let apl fs xs = concat_map (fun f -> map f xs) fs
...
let list_any p xs =
  let rec loop = function [] -> false | x :: xs -> p x || loop xs in
  loop xs
...
let rec is_forced xs =
  if Lazy.is_val xs then
    match next xs with Cons (_, xs) -> is_forced xs | Nil -> true
  else false
...
let find_all eq set xs =
  let rec f set xs =
    if Bl.is_empty set then []
    else
      match next xs with
      | Nil -> []
      | Cons (y, ys) ->
          let matches, rst = Bl.partition (fun x -> eq x y) set in
          if Bl.is_empty matches then f set ys else y :: f rst ys
  in
  f set xs
