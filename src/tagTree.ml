...
struct
  ...
  let rec to_list (Node (x, cs)) = x ^@^ concat_map to_list (map snd cs)

  let branch bs =
    if is_empty (drop 1 bs) then Node (bs, nil)
    else Node (nil, map (fun b -> (b, return b)) bs)

  let zero () = Node (nil, nil)

  let rec null (Node (xs, cs)) = is_empty xs && all null (map snd cs)
  ...
  let rec map_tree f (Node (xs, cs)) = Node (f xs, map2 (map_tree f) cs)
  ...
  let with_sibling_tags cs = map (fun (c, cs) -> (c, map fst cs)) (pinch cs)
  ...
  *)
  let combine, simplify =
    ...
    let rec simp_children ancestor_tags uncle_tags tagged_xs =
      lazy
        (let tagged_xs =
           filter
             (fun (tag, _) -> not (mem ~cmp:Tag.cmp tag uncle_tags))
             tagged_xs
         in
         let weaks =
           filter (fun (tag, _) -> mem ~cmp:Tag.cmp tag ancestor_tags) tagged_xs
         in
         if is_empty weaks then Node (nil, tagged_xs)
         else
           fold_right1 (comb ( ^@^ ) ancestor_tags uncle_tags) (map snd weaks))
    and simplify ancestor_tags uncle_tags (Node (x, tagged_xs)) =
      let x', tagged_xs' =
        dest_tree (simp_children ancestor_tags uncle_tags tagged_xs)
      in
      Node (x ^@^ x', map2 (simplify ancestor_tags uncle_tags) tagged_xs')
    and comb
          : 'a 'c.
            ('a LazyList.t -> 'b LazyList.t -> 'c) ->
            Tag.t LazyList.t ->
            Tag.t LazyList.t ->
                   ...
                   (fun ((tag, c), sibling_tags) ->
                     ( tag,
                       ...
                       comb f ancestor_tags uncle_tags c
                         (simp_children ancestor_tags uncle_tags tagged_ys) ))
                   (with_sibling_tags tagged_xs))) )
    in

    ((fun f tree_x tree_y -> comb f nil nil tree_x tree_y), simplify)
        ...
        (lazy
          (Node
             ( nil,
               map
                 (fun ((tag, c), rst) ->
                   (tag, simplify (singleton tag) (map fst rst) (join c)))
                 (pinch cs) )))
    in
    join (map_tree (fun xs -> fold_right lplus (zero ()) (map f xs)) t)
  ...
  let rec close_cases (Node (xs, cs)) =
    let descendents cs =
      any (fun (_, Node (xs', gcs)) -> is_non_empty xs' || is_non_empty gcs) cs
    in
    let ncs = map2 close_cases cs in
  ...
  let difference p t1 t2 =
    let rec filter_down (Node ((xs, ys), cs)) =
      Node
        ( LazyList.difference p xs ys,
          map2
            (fun c -> map_tree (fun xs' -> difference p xs' ys) (filter_down c))
            cs )
    in
  ...
  let rec nub p (Node (xs, cs)) =
    ...
    Node
      ( xs',
        map2
          (compose (map_tree (fun ys -> LazyList.difference p ys xs')) (nub p))
          cs )
  ...
  let rec with_tags (Node (xs, cs)) =
    Node
      ( map (fun x -> ([], x)) xs,
        map
          (fun (t, c) ->
            (t, map_tree (map (fun (ts, x) -> (t :: ts, x))) (with_tags c)))
          cs )

  let to_list (Node (xs, cs)) =
    map (fun x -> x) xs ^@^ concat_map (fun (_, c) -> to_list c) cs
  ...
  let rec to_forced (Node (x, cs)) =
    if is_forced cs then Node (x, map2 to_forced cs) else Node (x, nil)
end
